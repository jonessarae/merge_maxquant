...
Files must be from triple SILAC experiments.
...
def main(args):
    ...
    # check if provided files are MaxQuant proteinGroups text files - must contain Intensity columns
    try:
    ...
    # get info about experiments from Intensity column name
    # note this is from the first Intensity column only
    ...
    # check that files being merged are from the same group
    if m1_group != m2_group:
        print("The proteinGroups.txt files do not belong to the same group.")
        print("Please check the paths to the files.")
        sys.exit(0) # exit program

    # check that the mixes are not the same in both files
    if m1_mix == m2_mix:
    ...
    df_rows = df_rows[["Majority protein IDs"]].drop_duplicates() # remove duplicates
    df_rows = df_rows.sort_values(by="Majority protein IDs") # sort IDs
    df_rows = df_rows.reset_index(drop=True) # reset dataframe index
    ...
    # iterate through specific columns to merge them
    for i in range(1, length):
        ...
        if temp_df[temp_df.columns[i]].dtype == np.float64:
            ...
            # fill in values missing from either mix
